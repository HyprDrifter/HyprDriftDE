...
if [[ "$1" == "lock" ]]; then
  ...
  # Generate blurred wallpaper for lockscreen
  wall=$(grep '^wallpaper *= *' ~/.config/hypr/hyprpaper.conf | head -n 1 | cut -d',' -f2- | xargs realpath)
  if [[ -f "$wall" ]]; then
    mkdir -p ~/.cache/hyprlock
    convert "$wall" -blur 0x15 ~/.cache/hyprlock/blurred_wallpaper.png
  else
    echo ":: Wallpaper not found for blur."
  fi
